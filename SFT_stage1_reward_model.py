...
class Train_dataset(torch.utils.data.Dataset):
    ...
    def split_rating_text(self,text: str) -> tuple[str, str]:
        """
        ...
        # rfind() 从右侧开始查找，返回最后一次出现该子字符串的起始索引
        # 如果找不到，则返回 -1
        last_index = text.rfind(target)
        
        if last_index == -1:
            # 如果没有找到目标子字符串，默认 q 为原字符串，a 为空字符串
            # 你也可以根据需求改为抛出异常如 raise ValueError("未找到目标子串")
            return text, ""
        
        # 计算切割点：最后一次出现的起始索引 + 目标子串的长度
    ...
    def get_prompt(self, da):
        ...
        try:
            ...
            da = self.get_response(da)
            q , a = self.split_rating_text(da)
